...
<pre><code>    gateway.get_contacts(:type => :all, :sort => :name, :direction => :desc)
    gateway.get_contacts(:type => :all, :update_after => 1.month.ago) # modified since 1 month ago</code></pre>
...
<pre><code>    accounts_list = gateway.get_accounts_list
    ...
    all_output_tax_accounts = accounts_list.find_all_by_tax_type('OUTPUT')</code></pre>
