module XeroGateway
  class Contact
    ...
    attr_accessor :contact_id, :contact_number, :status, :name, :firstname, :lastname, :email, :addresses, :phones, :updated_at
    ...
    def self.from_xml(contact_element, gateway = nil)
      ...
      contact_element.children.each do |element|
        case(element.name)
          ...
          when "FirstName" then contact.firstname = element.text
          when "LastName" then contact.lastname = element.text
    ...
    def ==(other)
      [:contact_number, :status, :name, :firstname, :lastname, :email, :addresses, :phones].each do |field|
