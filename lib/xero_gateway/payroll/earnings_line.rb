module XeroGateway::Payroll
  class EarningsLine
    ...
    attr_accessor :number_of_units_per_week, :annual_salary, :rate_per_unit, :normal_number_of_units, :earnings_rate_id, :calculation_type, :number_of_units, :fixed_amount
    ...
    def to_xml(b = Builder::XmlMarkup.new)
      b.EarningsLine {
        ...
        b.FixedAmount self.fixed_amount if self.fixed_amount
      }
    end

    def self.from_xml(earnings_line_element, gateway = nil)
      earnings_line = EarningsLine.new
      earnings_line.gateway = gateway
      earnings_line_element.children.each do |element|
        case(element.name)
          ...
          when "FixedAmount" then earnings_line.fixed_amount = element.text
        end
      end
      earnings_line
    end
    ...
    def ==(other)
      [ :number_of_units_per_week, :annual_salary, :rate_per_unit, :number_of_units, :normal_number_of_units, :earnings_rate_id, :calculation_type, :fixed_amount ].each do |field|
